...
public class InventoryController {
	...
	public List<Inventory> getinventoryList(Model model) {
		...
		System.out.println("InventoryController.getinventoryList()"+inventoryList.get(0).toString());
		model.addAttribute("inventoryList", inventoryList);
		return inventoryList;
	}

	@RequestMapping(value = "/addInventory", method = RequestMethod.POST)
	public String addInventory(Model model, @RequestParam(value = "supplierId") Long supplierId,
			@RequestParam(value = "quantity") int quantity, @RequestParam(value = "price") double price,
			@RequestParam(value = "batchNum") int batchNum, @RequestParam(value = "productId") Long productId,
			@RequestParam(value = "sku") String sku, HttpSession httpSession) {
		...
		try {
			...
			/*
			 * if (product.getProductName().equals("") || quantity<=0 ||
			 * price<=0.0||supplier.getName().equals("")||salesman.getId().equals("")||sku.
			 * equals("")) { throw new Exception("Requied field is missing"); }
			 */
	...
	public ResponseEntity<?> getprice(@RequestParam(value = "sku") String sku) {
		Inventory inventory = inventoryDao.getInventoryBySku(sku);

		return new ResponseEntity<>(inventory, HttpStatus.OK);
	}

	@RequestMapping(value = "/delectInventoryById", method = RequestMethod.GET)
	public void delectProductById(@RequestParam(value = "id") Long id) {
		Inventory inventory = new Inventory(id);
		inventoryDao.removeInventory(inventory);
